...
def _print_summary(report, elapsed: float, output_dir: Path):
    ...
    if report.extraction_breakdown:
        method_table = Table(title="Extraction Method Breakdown", box=box.SIMPLE, show_header=True, header_style="bold")
    ...
    if report.chunk_type_breakdown:
        chunk_table = Table(title="Chunk Type Breakdown", box=box.SIMPLE, show_header=True, header_style="bold")
...
Examples:
  ...
  python -m catalog2md catalog.pdf -o ./output --min-tokens 256 --max-tokens 512
    ...
    parser.add_argument("input", type=Path, help="Path to a single PDF file or a directory of PDFs")
    parser.add_argument("-o", "--output", type=Path, required=True, help="Output directory for Markdown files and chunks")
    parser.add_argument("--api-key", type=str, default=None, help="Anthropic API key for Claude vision fallback (or set ANTHROPIC_API_KEY env var)")
    parser.add_argument("--force-method", type=str, choices=["docling", "pdfplumber", "claude_vision"], default=None, help="Force a specific extraction method (skip fallback cascade)")
    parser.add_argument("--min-tokens", type=int, default=512, help="Minimum chunk size in tokens (default: 512)")
    parser.add_argument("--max-tokens", type=int, default=1024, help="Maximum chunk size in tokens for text chunks (default: 1024)")
    parser.add_argument("--no-docling", action="store_true", help="Skip Docling even if installed (use pdfplumber as primary)")
