...
def flatten_merged_headers(headers, rows, parent_context=None):
    ...
    UNIT_EXPANSIONS = {
        ...
        "\u00b0F": ("Temperature", "\u00b0F"), "\u00b0C": ("Temperature", "\u00b0C"),
        "GPM": ("Flow Rate", "GPM"), "LPM": ("Flow Rate", "LPM"),
        "IN": ("Dimension", "in"), "MM": ("Dimension", "mm"),
        "LBS": ("Weight", "lbs"), "KG": ("Weight", "kg"),
        "CFM": ("Air Flow", "CFM"), "HP": ("Power", "HP"),
        "KW": ("Power", "kW"), "RPM": ("Speed", "RPM"),
        "BTU": ("Capacity", "BTU"),
    }
    for i, h in enumerate(flattened):
...
def validate_table(md):
    ...
    for i, row in enumerate(rows):
        if len(row) != col_count:
            issues.append(f"Row {i + 1} has {len(row)} columns, expected {col_count}")
    if rows:
        avg_cell_len = sum(sum(len(c) for c in row) / max(len(row), 1) for row in rows) / len(rows)
        for i, row in enumerate(rows):
